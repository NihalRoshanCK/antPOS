...
def create_items():
    ...
    _make_item(
        ITEM_BATCH, barcode=BARCODE_BATCH, item_name="AntPOS Batched Item",
        has_batch_no=1, rate=150.0,
    )
    _make_item(
        ITEM_SERIAL, barcode=BARCODE_SERIAL, item_name="AntPOS Serialised Item",
        has_serial_no=1, rate=200.0,
    )
    _make_item(
        ITEM_BATCH_SERIAL, barcode=BARCODE_BATCH_SERIAL,
        item_name="AntPOS Batched + Serialised Item",
        has_batch_no=1, has_serial_no=1, rate=250.0,
    )
...
def receive_stock(company, warehouse):
    ...
    _receive(
        ...
        qty=len(SERIALS_PLAIN), serial_no=SERIALS_PLAIN,
    )
    ...
    _receive(
        company, warehouse, ITEM_BATCH_SERIAL,
        qty=len(SERIALS_IN_BATCH_A),
        batch_no=f"{BATCH_A}-BS", serial_no=SERIALS_IN_BATCH_A,
    )
    _receive(
        company, warehouse, ITEM_BATCH_SERIAL,
        qty=len(SERIALS_IN_BATCH_B),
        batch_no=f"{BATCH_B}-BS", serial_no=SERIALS_IN_BATCH_B,
    )
...
def teardown():
    ...
    closing = frappe.get_all(
        "Ant Closing Shift", filters=shift_filter, fields=["name", "docstatus"]
    )
    opening = frappe.get_all(
        "Ant Opening Shift", filters=shift_filter, fields=["name", "docstatus"]
    )
