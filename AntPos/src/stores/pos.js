...
export const useInvoiceStore = defineStore('salesInvoice', () => {
  ...
  const cartSignature = computed(() => signatureOf(items.value, invoice.value, invoiceCustomer.value));
  // A submitted or paying invoice holds the saved figures; nothing to wait for.
  const totalsPending = computed(() =>
    items.value.length > 0 && !invoice.value?.docstatus && totalsFor.value !== cartSignature.value
  );
  // A replaced invoice (new sale, held sale, return, saved draft) carries
  // totals the current lines have not been checked against.
  watch(invoice, () => { totalsFor.value = null; }, { flush: 'sync' });
  ...
  function removeLine(line) {
    ...
    if (index === -1 && line?.custom_id != null) {
      index = items.value.findIndex((l) => String(l.custom_id) === String(line.custom_id));
    }
...
function signatureOf(lines, invoice, customer) {
  ...
  const parts = lines.map((l) => [
    l.custom_id, l.item_code, num(l.qty), num(l.rate), l.uom || '',
    l.batch_no?.value ?? l.batch_no ?? '', num(l.discount_percentage),
    (l.selected_serial_no || []).length,
  ].join('|'));
  parts.push([
    customer?.name || '', invoice?.is_return ? 1 : 0,
    num(invoice?._discount_amount), num(invoice?._additional_discount_percentage),
  ].join('|'));
  return parts.join(';');
}
