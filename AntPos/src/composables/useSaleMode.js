...
export function useSaleMode() {
    ...
    const defaultOn = computed(() => Boolean(profile.value.custom_set_sales_order));

    // Returns are credit notes; they never create an order.
    const available = computed(() =>
        Boolean(profile.value.custom_create_sales_order || defaultOn.value) && !invoiceStore.invoice?.is_return
    );

    // Only offer a choice when the profile allows one; a profile that only
    // sets the default always creates the order.
    const canChoose = computed(() => available.value && Boolean(profile.value.custom_create_sales_order));

    const asSalesOrder = computed({
        get: () => available.value && (invoiceStore.salesOrderChoice ?? defaultOn.value),
        set: (value) => {
    ...
    const ensureDeliveryDate = () => {
        if (!invoiceStore.invoice.delivery_date) invoiceStore.invoice.delivery_date = today();
    };

    return { available, canChoose, asSalesOrder, deliveryDate, ensureDeliveryDate };
}
