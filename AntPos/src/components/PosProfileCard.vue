<template>
  <!-- The till's POS profile and shift, at the foot of the sidebar (where
       ...
       Close shift. -->
  <Dropdown v-if="profile" :options="options" placement="right" side="right" align="end">
    <template #default="{ open }">
      <button
          ...
          :class="[
            isCollapsed ? 'justify-center border-transparent p-1.5' : 'gap-2.5 px-2 py-2',
            isCollapsed ? 'hover:bg-surface-gray-2' : open ? 'border-outline-gray-2 bg-surface-white shadow-sm' : 'border-outline-gray-1 bg-surface-white hover:border-outline-gray-2',
          ]"
          :aria-label="isCollapsed ? `${profile.name}, ${shiftLabel}` : undefined"
          :title="isCollapsed ? `${profile.name} · ${shiftLabel}` : undefined"
        >
          <span class="relative grid size-7 shrink-0 place-items-center rounded bg-surface-gray-2 text-ink-gray-7">
          ...
          <span v-if="!isCollapsed" class="min-w-0 flex-1">
            <span class="block truncate text-sm font-medium text-ink-gray-8">{{ profile.name }}</span>
            <span class="block truncate text-xs text-ink-gray-5">{{ shiftLabel }}</span>
          </span>
          <LucideChevronsUpDown v-if="!isCollapsed" class="size-4 shrink-0 text-ink-gray-5" />
        </button>
    </template>
  </Dropdown>
</template>
