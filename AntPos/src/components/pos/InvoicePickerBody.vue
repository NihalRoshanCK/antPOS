<template>
  <div class="flex min-h-0 flex-1 flex-col">
    <div class="shrink-0 border-b border-outline-gray-1 px-4 pb-3 pt-3 sm:px-5 sm:pt-4">
    ...
    <div class="min-h-0 flex-1 overflow-y-auto pos-scroll">
      <!-- First load -->
      <div v-if="loading && !invoices.length" class="space-y-1 p-2" aria-busy="true">
      ...
      <div v-else-if="!invoices.length" class="flex h-full flex-col items-center justify-center px-8 py-12 text-center">
        ...
        <p class="mt-3 text-base font-medium text-ink-gray-8">{{ searching ? 'No matches' : emptyTitle }}</p>
        <p class="mt-1 max-w-xs text-sm text-ink-gray-5">
          <template v-if="searching">
            Nothing matches “{{ search.trim() }}”. Try the invoice number or the customer's name.
          </template>
      ...
      <div v-else class="pb-2">
        <section v-for="group in groups" :key="group.label" :aria-label="group.label">
          <h3 class="sticky top-0 z-10 bg-surface-modal px-4 pb-1 pt-3 text-xs font-medium uppercase tracking-wide text-ink-gray-5 sm:px-5">
            {{ group.label }}
          </h3>
          <ul class="px-2 sm:px-3">
            <li v-for="invoice in group.invoices" :key="invoice.name">
              <button
                ...
                :class="[
                  opening === invoice.name ? 'bg-surface-gray-2' : 'hover:bg-surface-gray-2 active:bg-surface-gray-3',
              ...
              >
                ...
                <span class="min-w-0 flex-1">
                  <span class="flex min-w-0 items-center gap-1.5">
                    <span class="truncate text-base font-medium text-ink-gray-9">{{ customerOf(invoice) }}</span>
                    <span
                      v-if="showStatus && invoice.status"
                      class="shrink-0 rounded px-1.5 text-xs font-medium leading-5"
                      :class="STATUS_THEME[invoice.status] || 'bg-surface-gray-2 text-ink-gray-6'"
                    >
                      {{ invoice.status }}
                    </span>
                  </span>
                  <span class="num block truncate text-sm text-ink-gray-5">{{ metaOf(invoice) }}</span>
                </span>
                <span class="flex shrink-0 flex-col items-end">
                  <span class="num text-base font-semibold text-ink-gray-9">{{ money(invoice) }}</span>
                  <span v-if="itemsOf(invoice)" class="num text-sm text-ink-gray-5">{{ itemsOf(invoice) }}</span>
                </span>
                <span class="grid w-5 shrink-0 place-items-center text-ink-gray-4" aria-hidden="true">
                  <LoadingIndicator v-if="opening === invoice.name" class="h-4 w-4" />
                  <LucideChevronRight v-else class="h-4 w-4 transition-transform group-hover:translate-x-0.5" />
                </span>
              </button>
            </li>
          </ul>
        ...
        <div v-if="hasMore" class="px-4 pt-2 sm:px-5">
          <Button class="w-full" variant="subtle" :loading="loading" @click="$emit('load-more')">Show more</Button>
        </div>
      </div>
    </div>
    ...
    >
      <span class="num">{{ invoices.length }} {{ invoices.length === 1 ? 'invoice' : 'invoices' }}{{ hasMore ? ' shown' : '' }}</span>
...
function metaOf(invoice) {
  ...
  if (invoice.posting_time) parts.push(dayjsLocal(`2000-01-01 ${invoice.posting_time}`).format('h:mm A'))
  return parts.join(' · ')
}
