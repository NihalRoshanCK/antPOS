  ...
  <div class="flex items-end justify-between gap-4" :aria-busy="pending">
    <dl class="flex min-w-0 flex-wrap gap-x-5 gap-y-1 text-sm">
      <div>
        ...
        <dd class="num text-base font-medium text-ink-gray-8">{{ formatQty(qty) }}</dd>
      </div>
      <div>
        <dt class="text-ink-gray-5">Net</dt>
        <dd class="num text-base font-medium text-ink-gray-8 transition-opacity" :class="pending ? 'opacity-50' : ''">{{ money(net) }}</dd>
      </div>
      <div>
        <dt class="text-ink-gray-5">Tax</dt>
        <dd class="num text-base font-medium text-ink-gray-8 transition-opacity" :class="pending ? 'opacity-50' : ''">{{ money(tax) }}</dd>
      </div>
      <div v-if="discount">
    ...
    <div class="shrink-0 text-right">
      ...
      <div
        ...
        :class="[isReturn ? 'text-ink-red-4' : 'text-ink-gray-9', pending ? 'opacity-50' : '']"
      >
