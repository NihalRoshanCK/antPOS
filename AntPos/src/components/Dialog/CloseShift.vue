<template>
  <Dialog v-model="dialogVisible" :options="{ size: '3xl' }">
    <template #body-title>
    ...
    <template #body-content>
      <!-- Spinner at the top -->
      <div v-if="mode.isLoading" class="w-full flex justify-center mb-2">
      ...
      <div class="flex justify-end gap-2 mt-4">
        ...
        <Button
          variant="solid"
          @click="handleSubmit.fetch({ action: 'Submit' })"
          >Submit</Button
        >
      </div>
    </template>
...
const calculateTotals = () => {
  totals.value = {
    opening: data.value.reduce(
      (sum, row) => sum + (Number(row.opening) || 0),
      0,
    ),
    closing: data.value.reduce(
      (sum, row) => sum + (Number(row.closing) || 0),
      0,
    ),
    expected: data.value.reduce(
      (sum, row) => sum + (Number(row.expected) || 0),
      0,
    ),
    difference: data.value.reduce(
